# 邮箱验证流程文档

## 流程概述

新的验证流程：**先验证邮箱，再进行AI分析**

这样设计的优势：
- ✅ 避免无效邮箱浪费AI分析资源
- ✅ 用户体验更流畅（验证快，分析慢）
- ✅ 降低运营成本
- ✅ 防止恶意提交

## 完整流程

```
用户提交URL + 邮箱
    ↓
验证URL格式 ✓
    ↓
检查URL是否已存在 ✓
    ↓
生成6位验证码
    ↓
存储到 agent_submissions 表（agent_data = null）
    ↓
发送验证邮件 📧
    ↓
用户输入验证码
    ↓
验证验证码 ✓
    ↓
【开始AI分析】analyzeURL()
    ↓
提取Agent信息（名称、描述、特性等）
    ↓
创建Agent记录
    ↓
更新 agent_submissions（verified = true, agent_id）
    ↓
发送成功通知邮件 🎉
    ↓
返回Agent信息和链接
```

## API端点

### 1. POST /api/send-verification

**功能**：发送验证码（不做AI分析）

**请求**：
```json
{
  "url": "https://chat.openai.com/g/...",
  "email": "user@example.com"
}
```

**处理流程**：
1. 验证URL格式（validateURL）
2. 检查URL是否已存在
3. 生成6位验证码
4. 存储到 agent_submissions（agent_data = null）
5. 发送验证邮件
6. 返回成功消息

**响应**：
```json
{
  "success": true,
  "message": "验证码已发送到你的邮箱"
}
```

**错误处理**：
- 400: URL格式错误、邮箱格式错误
- 409: URL已存在
- 429: 请求过于频繁（速率限制）
- 500: 发送邮件失败

### 2. POST /api/verify-and-publish

**功能**：验证邮箱 → 分析URL → 上架Agent

**请求**：
```json
{
  "email": "user@example.com",
  "code": "123456"
}
```

**处理流程**：
1. 验证验证码（从 agent_submissions 查询）
2. 检查是否过期（10分钟有效期）
3. **开始AI分析**：analyzeURL(submission.url)
4. 提取Agent信息
5. 创建Agent记录
6. 更新 agent_submissions（verified = true）
7. 发送成功通知邮件
8. 返回Agent信息

**响应**：
```json
{
  "success": true,
  "message": "🎉 Agent已成功上架！",
  "agent": {
    "id": "uuid",
    "name": "Agent名称",
    "slug": "agent-slug",
    "url": "/agents/agent-slug"
  }
}
```

**错误处理**：
- 400: 验证码无效或已过期
- 422: URL分析失败
- 500: 创建Agent失败

## 数据库表

### agent_submissions

```sql
CREATE TABLE agent_submissions (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  email VARCHAR(255) NOT NULL,
  url TEXT NOT NULL,
  verification_code VARCHAR(6) NOT NULL,
  expires_at TIMESTAMP WITH TIME ZONE NOT NULL,
  verified BOOLEAN DEFAULT false,
  agent_id UUID REFERENCES agents(id),
  agent_data JSONB,  -- 验证后才填充
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  UNIQUE(email, url)
);
```

**字段说明**：
- `agent_data`: 验证前为 null，验证后存储AI分析结果
- `verified`: 邮箱是否已验证
- `agent_id`: 创建Agent后的ID引用

## 前端组件

### PublishAgentSection

**步骤1: 表单（form）**
- 输入URL和邮箱
- 同意服务条款
- 点击"发送验证码"
- 调用 `/api/send-verification`

**步骤2: 验证（verify）**
- 显示邮箱地址
- 输入6位验证码
- 点击"验证并上架"
- 调用 `/api/verify-and-publish`
- 显示加载状态（AI分析中...）

**步骤3: 成功（success）**
- 显示成功消息
- 显示Agent名称
- 提供查看链接
- 可继续发布其他Agent

## 邮件模板

### 验证码邮件

```
主题: 验证你的邮箱 - Agent Signals

你好！

你的验证码是：123456

此验证码将在10分钟后过期。

如果这不是你的操作，请忽略此邮件。
```

### 成功通知邮件

```
主题: 🎉 你的Agent已成功上架！

你好！

你提交的Agent "Agent名称" 已成功上架到 Agent Signals 平台。

查看你的Agent: https://agentsignals.ai/agents/agent-slug

感谢你的贡献！
```

## 安全措施

### 1. 速率限制
- 每邮箱每分钟最多3次请求
- 使用内存Map存储（生产环境建议用Redis）

```typescript
const rateLimitMap = new Map<string, { count: number; resetTime: number }>()

function checkRateLimit(key: string, maxRequests = 3, windowMs = 60000): boolean {
  // 实现逻辑...
}
```

### 2. 验证码过期
- 10分钟有效期
- 过期后需重新获取

### 3. URL去重
- 检查URL是否已存在
- 防止重复提交

### 4. 邮箱验证
- 必须验证邮箱才能上架
- 防止垃圾提交

## 性能优化

### 1. 异步处理
- 成功通知邮件异步发送
- 不阻塞响应

```typescript
sendPublishSuccessEmail(email, agent.name, agent.slug).catch(err => {
  console.error('Send success email failed:', err)
})
```

### 2. 资源节约
- 验证前不做AI分析
- 只对有效邮箱进行分析
- 预计节省 30-50% AI成本

### 3. 用户体验
- 验证码发送快（< 1秒）
- AI分析慢（5-10秒）
- 分离两个步骤，体验更好

## 错误处理

### 常见错误

1. **验证码发送失败**
   - 检查 Resend API Key
   - 检查邮箱格式
   - 查看 Resend Dashboard 日志

2. **验证码无效**
   - 检查是否过期
   - 检查邮箱和验证码是否匹配
   - 确认数据库记录存在

3. **AI分析失败**
   - 检查 OpenRouter API Key
   - 检查URL是否可访问
   - 查看错误日志

4. **Agent创建失败**
   - 检查 Supabase 连接
   - 检查数据格式
   - 查看数据库约束

## 测试

### 单元测试

```bash
# 测试验证码发送
npm test -- send-verification --run

# 测试验证和发布
npm test -- verify-and-publish --run
```

### 手动测试

1. 访问首页 `http://localhost:3000`
2. 滚动到"发布你的AI Agent"区域
3. 输入测试URL和邮箱
4. 检查邮箱收到验证码
5. 输入验证码
6. 等待AI分析完成
7. 验证Agent已创建

### 测试用例

| 场景 | 输入 | 期望结果 |
|------|------|----------|
| 有效提交 | 正确URL + 邮箱 | 发送验证码 |
| 无效URL | 错误URL格式 | 400错误 |
| 无效邮箱 | 错误邮箱格式 | 400错误 |
| URL已存在 | 已存在的URL | 409错误 |
| 速率限制 | 连续4次请求 | 429错误 |
| 验证码错误 | 错误验证码 | 400错误 |
| 验证码过期 | 11分钟后验证 | 400错误 |
| 成功上架 | 正确验证码 | 创建Agent |

## 监控指标

建议监控以下指标：

1. **转化率**
   - 发送验证码数
   - 验证成功数
   - 转化率 = 验证成功 / 发送验证码

2. **失败率**
   - 邮件发送失败率
   - AI分析失败率
   - Agent创建失败率

3. **性能**
   - 验证码发送耗时
   - AI分析耗时
   - 端到端耗时

4. **成本**
   - AI API调用次数
   - 邮件发送次数
   - 每个Agent的成本

## 未来优化

1. **批量处理**
   - 支持批量提交URL
   - 异步队列处理

2. **智能去重**
   - 检测相似URL
   - 合并重复Agent

3. **质量控制**
   - 人工审核机制
   - 自动质量评分

4. **用户管理**
   - 用户账号系统
   - 提交历史记录

## 总结

新的验证流程实现了：

✅ **先验证邮箱，再AI分析** - 节省资源  
✅ **清晰的三步流程** - 用户体验好  
✅ **完善的错误处理** - 稳定可靠  
✅ **安全措施** - 防止滥用  
✅ **性能优化** - 快速响应  

这个流程既保证了数据质量，又优化了成本和用户体验！
